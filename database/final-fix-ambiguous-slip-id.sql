-- Final fix for ambiguous slip_id reference
...
CREATE OR REPLACE FUNCTION public.safe_delete_custodian_slip_v3(slip_id_param uuid)
RETURNS boolean AS $$
DECLARE
  can_delete boolean;
  item_record record;
...
RETURNS boolean AS $$
BEGIN
  RETURN public.safe_delete_custodian_slip_v3(slip_id);
END;
$$ LANGUAGE plpgsql;
...
FROM pg_proc 
WHERE proname IN ('safe_delete_custodian_slip', 'safe_delete_custodian_slip_v3', 'can_delete_custodian_slip')
