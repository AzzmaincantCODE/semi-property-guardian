-- Force Delete Test Data - For Development/Testing Only
-- This will delete custodian slips and clean up all related data
-- ⚠️ WARNING: Only use this for testing/development data

-- Step 1: Release all inventory items from custodian slip assignments first
...
-- Step 2: Delete all custodian slip items (this removes foreign key references)
DELETE FROM public.custodian_slip_items;

-- Step 3: Delete all property card entries related to custodian slips
DELETE FROM public.property_card_entries 
WHERE related_slip_id IN (
  SELECT id FROM public.custodian_slips
);

-- Step 4: Delete all custodian slips
