-- PROPER FIX for draft slip deletion
-- This handles the fact that draft slips don't have property card entries yet
...
-- Step 2: Create a proper function that handles draft slips correctly
...
BEGIN
  ...
  -- For draft slips, we don't need to check property card entries
  -- because they don't exist yet. Only issued slips have property card entries.
...
-- Step 3: Create the can_delete function that handles draft slips correctly
...
BEGIN
  ...
  -- For draft slips, we can always delete them
  -- because they don't have property card entries yet
  RETURN true;
END;
$$ LANGUAGE plpgsql;
...
-- Step 8: Verify the functions were created successfully
SELECT 
