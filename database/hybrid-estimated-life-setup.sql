-- Hybrid Estimated Useful Life Setup
-- Adds manual override capability for estimated useful life calculation

-- Add estimated_useful_life_override column to inventory_items
ALTER TABLE inventory_items 
ADD COLUMN IF NOT EXISTS estimated_useful_life_override DECIMAL(5,2);

-- Add estimated_useful_life_override column to custodian_slip_items  
ALTER TABLE custodian_slip_items
ADD COLUMN IF NOT EXISTS estimated_useful_life_override DECIMAL(5,2);

-- Create function to calculate hybrid estimated useful life
...
-- Create trigger to auto-calculate estimated useful life when items are created/updated
CREATE OR REPLACE FUNCTION trg_calculate_estimated_life()
RETURNS TRIGGER AS $$
DECLARE
    v_calculation RECORD;
BEGIN
    -- Only calculate if estimated_useful_life is not already set or if manual override changed
    IF NEW.estimated_useful_life IS NULL OR 
...
-- Update existing inventory items to calculate their estimated useful life
...
WHERE estimated_useful_life IS NULL;
