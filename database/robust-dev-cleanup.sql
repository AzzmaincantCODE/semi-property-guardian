-- Robust Development Cleanup - Handles ALL Foreign Key Constraints
-- This version safely removes all data while respecting foreign key relationships
...
-- Step 2: Disable all triggers and constraints temporarily
...
-- Step 3: Clear foreign key references in correct order
...
-- Clear any other references to custodian slip items
UPDATE property_card_entries
SET 
...
-- Step 4: Reset all inventory items to clean state
...
-- Step 5: Delete custodian slip data in correct order
-- Delete custodian slip items first (child table)
DELETE FROM custodian_slip_items;

-- Delete custodian slips (parent table)
DELETE FROM custodian_slips;

-- Step 6: Clear any remaining property card entries
-- (This should be safe now since we cleared the references)
